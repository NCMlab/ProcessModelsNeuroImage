function fit = subfnFitRegressModelPCs(data)
...
%        fit = [ones(NSub,1) data.M data.X data.COV]*betaHat;
    case '14'
        NSSF = size(data.M,2);
        NMod = size(data.V,2);
        ...
        [betaHat ,FVAL,EXITFLAG,OUTPUT] = fminsearch('subfnRegressPCs',beta0,options,data);
