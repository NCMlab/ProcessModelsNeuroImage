function varargout = jrs_results_ui(varargin)
% User interface for SPM/PPM results: Display and analysis of regional effects
% FORMAT [hReg,xSPM,SPM] = jrs_results_ui
%
...
%                           ----------------
%
% In addition to setting up the results section, jrs_results_ui.m sets
...
% $Id: jrs_results_ui.m 816 2007-05-24 19:14:01Z karl $
...
%
% FORMAT jrs_results_ui('DrawButts',hReg,DIM,Finter,WS,FS)
% Draw GUI buttons
% hReg    - handle of XYZ registry object
% DIM     - 3 vector of image X, Y & Z dimensions
...
%
% FORMAT hFxyz = jrs_results_ui('DrawXYZgui',M,DIM,xSPM,xyz,Finter)
...
%
% FORMAT jrs_results_ui('EdWidCB')
% Callback for editable XYZ control widgets
%
% FORMAT jrs_results_ui('UpdateSPMval',hFxyz)
% FORMAT jrs_results_ui('UpdateSPMval',UD)
...
%
% FORMAT xyz = jrs_results_ui('GetCoords',hFxyz)
...
%
% FORMAT [xyz,d] = jrs_results_ui('SetCoords',xyz,hFxyz,hC)
...
% NB: When using the results section, should use XYZregistry to get/set location
%
% FORMAT hFxyz = jrs_results_ui('FindXYZframe',h)
...
%
% FORMAT hMP = jrs_results_ui('LaunchMP',M,DIM,hReg,hBmp)
% Prototype callback handler for integrating MultiPlanar toolbox
%
% FORMAT jrs_results_ui('Delete',h)
...
%	[SPM,xSPM] = spm_getSPM;
    ...
    %======================================================================
        ...
        %------------------------------------------------------------------
        hFxyz      = jrs_results_ui('DrawXYZgui',M,DIM,varargin{4},xyz,Finter);
        spm_XYZreg('XReg',hReg,hFxyz,'jrs_results_ui');

        %-Set up buttons for results functions
        %------------------------------------------------------------------
        jrs_results_ui('DrawButts',hReg,DIM,Finter,WS,FS);
...
%             'Tag','plotButton')
        ...
        hHelp  = uicontrol(Finter,'Style','PushButton','String','?',...
            ...
            'Callback','spm_help(''jrs_results_ui'')',...
            'Interruptible','on','Enable','on',...
    ...
    %======================================================================
        % hFxyz = jrs_results_ui('DrawXYZgui',M,DIM,xSPM,xyz,Finter)
        ...
        hX   = uicontrol(Finter,'Style','Edit','String',sprintf('%.2f',xyz(1)),...
            ...
            'Tag','hX',...
            'Callback','jrs_results_ui(''EdWidCB'')');
    ...
    %======================================================================
        ...
        if abs(xyz(d)-nxyz(d))>0
            ...
            jrs_results_ui('UpdateSPMval',UD)
        end

    %======================================================================
    case 'updatespmval'                           %-Update SPM value in GUI
    %======================================================================
        % jrs_results_ui('UpdateSPMval',hFxyz)
        % jrs_results_ui('UpdateSPMval',UD)
    ...
    %======================================================================
        % xyz = jrs_results_ui('GetCoords',hFxyz)
        if nargin<2, hFxyz='Interactive'; else, hFxyz=varargin{2}; end
        hFxyz     = jrs_results_ui('FindXYZframe',hFxyz);
    ...
    %======================================================================
        % [xyz,d] = jrs_results_ui('SetCoords',xyz,hFxyz,hC)
        if nargin<4, hC=0; else, hC=varargin{4}; end
        if nargin<3, hFxyz=jrs_results_ui('FindXYZframe'); else, hFxyz=varargin{3}; end
    ...
    %======================================================================
        % hFxyz = jrs_results_ui('FindXYZframe',h)
    ...
    %======================================================================
        ...
        h4 = uicontrol(Finter,'Style','PopUp',...
            ...
            'Callback','jrs_results_ui(''PlotUiCB'')',...
            'Interruptible','on','Enable','on',...
        ...
        set(hGraphUIbg,'UserData',...
            ...
            'DeleteFcn','jrs_results_ui(''Delete'',get(gcbo,''UserData''))')
    ...
    %======================================================================
        % Fgraph = jrs_results_ui('Clear',F,mode)
    ...
    %======================================================================
        % hMP = jrs_results_ui('LaunchMP',M,DIM,hReg,hBmp)
        ...
        set(hBmp,'DeleteFcn','jrs_results_ui(''delete'',get(gcbo,''UserData''))')
