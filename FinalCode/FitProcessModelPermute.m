...
for j = 1:NumberOfPaths
    ...
    elseif length(ResultPathSE) == 4
        Results.PathsSE{j} = sqrt(...
        (prod(ResultPathbetas([1 2 3]).^2).*(ResultPathSE(4).^2)) + ...
        (prod(ResultPathbetas([1 2 4]).^2)).*(ResultPathSE(3).^2) + ...
        (prod(ResultPathbetas([1 3 4]).^2)).*(ResultPathSE(2).^2) + ...
        (prod(ResultPathbetas([2 3 4]).^2)).*(ResultPathSE(1).^2));
