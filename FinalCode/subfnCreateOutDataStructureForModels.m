...
for j = 1:Nmed
    ...
    for k = 1:length(Model1FieldNames)
        if isempty(strmatch(Model1FieldNames(k),'Outcome')) && isempty(strmatch(Model1FieldNames(k),'Model'))
            ...
            OutData{index}.dataType = 16;
            index = index + 1;
            OutData{index}.name = ['Model1_Med' num2str(j) '_' Model1FieldNames{k} '_p'];
            OutData{index}.data = zeros(Nvoxels,1);
            OutData{index}.field = ['Model1{' num2str(j) '}.' Model1FieldNames{k} '.p'];
            OutData{index}.dataType = 16;
            index = index + 1;
        end
    end
...
for k = 1:length(Model2FieldNames)
    if isempty(strmatch(Model2FieldNames(k),'Outcome')) && isempty(strmatch(Model2FieldNames(k),'Model'))
        ...
        OutData{index}.dataType = 16;
        index = index + 1;
        OutData{index}.name = ['Model2_' Model2FieldNames{k} '_p'];
        OutData{index}.data = zeros(Nvoxels,1);
        OutData{index}.field = ['Model2.' Model2FieldNames{k} '.p'];
        OutData{index}.dataType = 16;
        index = index + 1;
    end
end
...
for k = 1:length(Model3FieldNames)
    if isempty(strmatch(Model3FieldNames(k),'Outcome')) && isempty(strmatch(Model3FieldNames(k),'Model'))
        ...
        OutData{index}.dataType = 16;
        index = index + 1;
        OutData{index}.name = ['Model3_' Model3FieldNames{k} '_p'];
        OutData{index}.data = zeros(Nvoxels,1);
        OutData{index}.field = ['Model3.' Model3FieldNames{k} '.p'];
        OutData{index}.dataType = 16;
        index = index + 1;
    end
end
