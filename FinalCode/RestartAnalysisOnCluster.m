...
F = dir(fullfile(SelectedPath,'Results','Results_*.mat'));
if ~(AnalysisParameters.NJobSplit == length(F))
    ...
    for i = 1:AnalysisParameters.NJobSplit
        if ~isempty(dir(fullfile(SelectedPath,'Results',sprintf('Results_%04d.mat',i))))
            FinishedData(i) = 1;
        end
    end
    for i = 1:AnalysisParameters.NJobSplit
        if ~FinishedData(i)
            jobPath = fullfile(OutFolder,'JobFiles',sprintf('job_%04d.sh',i));
            %    Str = ['! qsub  ' jobPath];
